...
import productImage from "../assets/product_duvet.jpg";

export default function DuvetCovers() {
  const { t } = useTranslation();
  const products = t("duvetCovers.items", { returnObjects: true });

  return (
    <main className="min-h-screen bg-[#f4eadc] px-5 pb-24 pt-32 sm:px-8 lg:px-12">
      <div className="mx-auto max-w-7xl">
        <p className="mb-4 text-[13px] uppercase tracking-[3px] text-[#b18a42]">{t("duvetCovers.eyebrow")}</p>
        <h1 className="mb-5 font-['Cormorant_Garamond'] text-5xl font-semibold text-[#8d6a2b] sm:text-6xl">{t("duvetCovers.title")}</h1>
        <p className="mb-12 max-w-2xl text-lg leading-relaxed text-gray-500">{t("duvetCovers.description")}</p>
          ...
          {products.map((product, index) => (
            <article key={product.name} className="overflow-hidden rounded-2xl border border-[#e1d3bb] bg-white/50">
              ...
              <div className="p-6">
                ...
                <p className="mt-5 font-medium text-gray-700">${[169, 189, 179][index]}</p>
              </div>
            </article>
          ))}
        </div>
