...
// coverMaterial stores the English key ("satin"/"stripe"/"cotton") so
// filtering logic stays language-independent — only the displayed
// label changes with t().
const DUVETS = [
  {
    id: 1,
    coverMaterial: "satin",
    fillMaterial: "Microfiber",
    price: 189,
    name: "Classic Satin Duvet",
    description: "Smooth satin finish, hotel-weight fill.",
  },
  {
    id: 2,
    ...
    fillMaterial: "Microfiber",
    price: 169,
    name: "Striped Weave Duvet",
    description: "Subtle woven stripe texture.",
  },
  {
    id: 3,
    ...
    fillMaterial: "Microfiber",
    price: 199,
    name: "Pure Cotton Duvet",
    description: "Breathable cotton cover, soft and durable.",
  },
];
...
export default function Duvet() {
  ...
  const filteredDuvets =
    selectedMaterial === "all"
      ? DUVETS
      : DUVETS.filter((item) => item.coverMaterial === selectedMaterial);
    ...
    <section className="bg-[#f4eadc] min-h-screen pt-32 pb-20 px-8 sm:px-10 lg:px-12">
      <div className="mx-auto max-w-7xl">
        ...
        <p className="text-gray-500 text-lg max-w-2xl leading-relaxed mb-10">
          {t("duvets.description")}
        </p>

        {/* Cover material filter buttons */}
              ...
              >
                <span
                  className={`w-9 h-9 rounded-full shrink-0 border ${
                    isActive ? "border-[#8d6a2b]" : "border-[#d3b98a]"
                  }`}
                ...
                >
                  {t(`duvets.materials.${material.key}`)}
                </span>
              </button>
            );
          })}
        ...
        <p className="text-[12px] tracking-[1px] text-gray-400 mb-10">
          {t("duvets.filter_note")}
        </p>

        {/* Product grid or empty state */}
        {filteredDuvets.length > 0 ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredDuvets.map((duvet) => (
              <div
                key={duvet.id}
                ...
                <div className="p-6">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-[11px] tracking-[2px] text-[#b18a42]">
                      {t(`duvets.materials.${duvet.coverMaterial}`)}{" "}
                      {t("duvets.cover_label")}
                    </span>
                    <span className="text-gray-300">•</span>
                    <span className="text-[11px] tracking-[2px] text-gray-400">
                      {duvet.fillMaterial.toUpperCase()} {t("duvets.fill_label")}
                    </span>
                  </div>
          ...
          <div className="flex flex-col items-center justify-center text-center py-24 border border-dashed border-[#d3b98a] rounded-2xl">
            <p className="font-['Cormorant_Garamond'] text-3xl text-[#8d6a2b] mb-3">
              {t("duvets.sorry.heading")}
            </p>
            <p className="text-gray-500 max-w-md">
              {t("duvets.sorry.message", {
                material: t(`duvets.materials.${selectedMaterial}`),
              })}
            </p>
          </div>
        )}
