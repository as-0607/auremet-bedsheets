...
export default function Promise({
  ...
  const heading = small_header || t("promise.heading");
  const title = big_header || t("promise.title");
  const promiseDetails = promise_details || [
    t("promise.promiseDetails.0"),
    t("promise.promiseDetails.1"),
    t("promise.promiseDetails.2"),
    t("promise.promiseDetails.3"),
  ];
  return (
    <div
      className={
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            <span className="font-['Cormorant_Garamond'] text-[46px] leading-none font-normal my-2">
              {t("promise.year")}
            </span>
            <div className="flex flex-col gap-[5px] text-[13px] font-medium tracking-[3px]">
              <span>{t("promise.year_span1")}</span>
              <span>{t("promise.year_span2")}</span>
            </div>
          </div>
          <img
            src={promise}
        ...
        <div className="lg:w-[50%]  pt-20">
            ...
            {promiseDetails.map((detail, index) => (
              <div key={index} className="flex items-center gap-3">
                <div className="h-6 w-6 rounded-full bg-[#f4eadc] text-center">
                  <span className="text-[#8d6a2b] text-sm">{t("promise.right_check")}</span>
                </div>
          ...
          >
            {standalone ? t("why_promise.button") :   t("promise.button")}
          </Link>
        </div>
      </div>
    </div>
